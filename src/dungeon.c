...
// Lookup tables for the boss heartbeat (ported from asm/fight.asm:4125).
// squares[i] = i^2 for i in 0..15 (dx^2 / dy^2, pre-squared deltas).
static const uint8_t heartbeat_squares[16] = {
    0, 1, 4, 9, 16, 25, 36, 49, 64, 81, 100, 121, 144, 169, 196, 225
};

// distance_attenuation: indexed by dx^2+dy^2 (0..255), maps the squared
// distance to the heartbeat volume (0x0F = loudest when right next to the
// door, 0x06 = faintest at the edge of range). Copied verbatim from the
// original 256-byte table.
static const uint8_t heartbeat_distance_attenuation[256] = {
...
// Port of asm/fight.asm:774E update_boss_heartbeat_volume.
// Computes the boss-heartbeat volume from the hero's distance to the door
// guarding the Tear.  The volume (0x00 = silence, 0x06..0x0F = audible, louder
// when closer) is stored in ADDR_HEARTBEAT_VOLUME and picked up by
// sound-manager.js which loops assets/sfx/heartbeat.ogg at that volume.
void update_boss_heartbeat_volume(void)
{
    // tear_x == 0xFFFF means the boss is dead and the Tear already acquired.
    if (MEM16(ADDR_TEAR_X) == 0xFFFF) goto distance_big;

    // is_in_proximity_window: relative column of the door inside the 36-wide
    // proximity window (world-wrap aware). Outside the window -> no beat.
    uint16_t rel;
    {
        uint16_t tear_x = MEM16(ADDR_TEAR_X);
        uint16_t prox_left = MEM16(ADDR_PROXIMITY_MAP_LEFT_COL);
        if (tear_x >= prox_left) {
            rel = (uint16_t)(tear_x - prox_left);
        } else if (tear_x > 35) {
            goto distance_big;               // 35 < tear_x < prox_left
        } else {
            rel = (uint16_t)(MEM16(ADDR_MAP_WIDTH) - prox_left + tear_x);
        }
    }
    if (rel > 35) goto distance_big;

    // Horizontal distance: |hero column - door column|.
    uint8_t hero_col = MEM8(ADDR_HERO_X_VIEW) + 4;
    int dx = (int)hero_col - (int)rel;
    if (dx < 0) dx = -dx;

    // Vertical distance, wrapping the 64-row map.
    uint8_t hero_y = MEM8(ADDR_HERO_Y);
    uint8_t tear_y = MEM8(ADDR_TEAR_Y);
    uint8_t dy_fwd = (uint8_t)(hero_y - tear_y) & 0x3F;
    uint8_t dy_rev = (uint8_t)(tear_y - hero_y) & 0x3F;
    uint8_t dy = (dy_rev < dy_fwd) ? dy_rev : dy_fwd;

    if (dx >= 16 || dy >= 16) goto distance_big;

    uint8_t dist2 = heartbeat_squares[dx] + heartbeat_squares[dy];
    if (dist2 < heartbeat_squares[dx]) goto distance_big; // dist^2 > 255 (carry)

    MEM8(ADDR_HEARTBEAT_VOLUME) = heartbeat_distance_attenuation[dist2];
    return;
