 ...
 *
 *   0 -> "turtle" - a monster that clings to a surface and is driven by
 ...
 *        up with the hero.
 *   1 -> "green_egg" - a monster that can link up with a second, partner
 ...
 *   2/3 -> "icicle" - a dormant trap: it sits still until the hero
 ...
 *   4 -> "arrow" - a monster confined to a narrow band of x positions
 *        that walks a small state machine of preset directions
 *        (arrow_table_left/right, indexed by ai_state) trying up to 5
 ...
 *   the AI logic itself.
 * - trajectory_right/left and arrow_table_left/right, by contrast,
 ...
 * by a rotated slice of monster.ai_state, used by turtle once it drops
 ...
 * tried in turn by arrow_try_directions() until one succeeds. Facing
 * left uses arrow_table_left, facing right uses arrow_table_right. */
static const uint8_t arrow_table_left[8 * 15] = { /* unk_A756 */
...
static const uint8_t arrow_table_right[8 * 15] = { /* unk_A7CE */
 ...
 */
static void turtle_ai(uint16_t m);
static void green_egg_ai(uint16_t m);
static void icicle_ai(uint16_t m);
static void arrow_ai(uint16_t m);

/* turtle helpers */
static void turtle_landed(uint16_t m);           /* loc_A2B0 */
static void turtle_patrol(uint16_t m);           /* loc_A2BF */
static void turtle_aligned(uint16_t m);          /* loc_A2EF */
static void turtle_go_west(uint16_t m);          /* loc_A302 */
static void turtle_go_east(uint16_t m);          /* loc_A313 */
static void turtle_snap_forward(uint16_t m);     /* loc_A324, ai_state bit 0x1 */
static void turtle_snap_backward(uint16_t m);    /* loc_A346, ai_state bit 0x2 */
static void turtle_bounce_dash(uint16_t m);      /* loc_A368, ai_state bit 0x4 */
static void turtle_settle(uint16_t m);           /* loc_A3CD, ai_state bit 0x8 */
static void turtle_settle_step2(uint16_t m);     /* loc_A3F8 */
static void turtle_settle_trajectory(uint16_t m);/* loc_A412 */

/* green_egg helpers */
static void green_egg_common(uint16_t m);           /* loc_A4EC */
static void green_egg_move(uint16_t m);             /* loc_A4F9 */
static void green_egg_fall_and_patrol(uint16_t m);  /* loc_A514 */
static void green_egg_bounce_west(uint16_t m);      /* loc_A54B */
static void green_egg_bounce_east(uint16_t m);      /* loc_A557 */
static void green_egg_linked_move(uint16_t m);      /* loc_A56C */
static int  green_egg_check_room(uint16_t prox, uint16_t offset); /* sub_A679 */
static void green_egg_grow_left(uint16_t m, uint16_t partner, uint16_t prox);  /* loc_A5C1 */
static void green_egg_grow_right(uint16_t m, uint16_t partner, uint16_t prox); /* loc_A618 */
static void green_egg_finish_link(uint16_t m, uint16_t partner, uint8_t saved_layer_val); /* loc_A64C */

/* icicle helpers */
static void icicle_active(uint16_t m);           /* loc_A6DB */

/* arrow helpers */
static uint8_t arrow_try_directions(uint16_t m); /* sub_A71C */
...
void Monster_AI_4(uint16_t m)
{
    switch (MEM8(m+4) & 0x0F) { // .flags
        case 0: turtle_ai(m); return;
        case 1: green_egg_ai(m); return;
        case 2: icicle_ai(m); return;
        case 3: icicle_ai(m); return; // original jump table: cases 2 and 3 share loc_A6B1
        case 4: arrow_ai(m); return;
        default:
 ...
 */
static void turtle_ai(uint16_t m) // loc_A281
{
    ...
    if (MEM8(m+9) & 8) { // .ai_state
        turtle_settle(m); // loc_A3CD
        return;
    }
    if (MEM8(m+9) & 4) { // .ai_state
        turtle_bounce_dash(m); // loc_A368
        return;
    }

    if (move_monster_S(m)) return; // still falling, nothing more this frame

    turtle_landed(m); // loc_A2B0
}

/* loc_A2B0: grounded - dispatch on the remaining two ai_state bits. */
static void turtle_landed(uint16_t m)
{
    if (MEM8(m+9) & 1) { // .ai_state
        turtle_snap_forward(m); // loc_A324
        return;
    }
    if (MEM8(m+9) & 2) { // .ai_state
        turtle_snap_backward(m); // loc_A346
        return;
    }
    turtle_patrol(m); // loc_A2BF
}
 ...
 * direction. */
static void turtle_patrol(uint16_t m)
{
    ...
    if (hero_y == my_y || ((hero_y + 1) & 0x3F) == my_y) {
        turtle_aligned(m); // loc_A2EF
        return;
    }

    if (MEM8(m+5) & 0x80) { // .ai_flags: facing right
        turtle_go_east(m); // loc_A313
    } else {
        turtle_go_west(m); // loc_A302
    }
}
...
static void turtle_aligned(uint16_t m)
{
    if ((get_random() & 3) == 0) {
        MEM8(m+9) = 5; // .ai_state
    }
...
static void turtle_go_west(uint16_t m)
{
...
static void turtle_go_east(uint16_t m)
{
...
static void turtle_snap_forward(uint16_t m)
{
...
/* loc_A346: ai_state bit 0x2 - mirror of turtle_snap_forward, counting
 * down instead of up. */
static void turtle_snap_backward(uint16_t m)
{
    uint8_t low = MEM8(m+6) & 0x0F; // .anim_counter
    if (low >= 0x0C) {
        MEM8(m+6) = 0x0B;
...
static void turtle_bounce_dash(uint16_t m)
{
 ...
 * to turtle_settle_step2. */
static void turtle_settle(uint16_t m)
{
    ...
    if (MEM8(m+0xA) & 1) { // .ai_timer: initial delay already elapsed
        turtle_settle_step2(m); // loc_A3F8
        return;
    }
...
static void turtle_settle_step2(uint16_t m)
{
    if (MEM8(m+0xA) & 4) { // .ai_timer
        turtle_settle_trajectory(m); // loc_A412
        return;
    }
    MEM8(m+0xA) |= 4;
    if (MEM8(m+0xA) & 8) {
...
static void turtle_settle_trajectory(uint16_t m)
{
 ...
 */
static void green_egg_ai(uint16_t m) // loc_A466
{
    if (MEM8(m+8) == 0) MEM8(m+8) = 0x10; // .hp

    if (!(MEM8(m+5) & 0x20)) { // .ai_flags: not hit
        green_egg_common(m); // loc_A4EC
        return;
    }
 ...
 * set, e.g. by green_egg_grow_* below). */
static void green_egg_common(uint16_t m)
{
    int bit_set = (MEM8(m+9) & 1) != 0; // .ai_state
    MEM8(m+9) &= 0xFE;
    if (bit_set) return;

    green_egg_move(m); // loc_A4F9
}

/* loc_A4F9: unlinked monsters just walk; linked ones run the growth /
 * repositioning logic instead. */
static void green_egg_move(uint16_t m)
{
    if (MEM8(m+7) & 0x40) { // linked to a partner
        green_egg_linked_move(m); // loc_A56C
        return;
    }
...
static void green_egg_fall_and_patrol(uint16_t m)
{
    ...
    if (!aligned) {
        if (MEM8(m+5) & 0x80) { // .ai_flags: facing right
            green_egg_bounce_east(m); // loc_A557
        } else {
            green_egg_bounce_west(m); // loc_A54B
        }
        return;
    }
...
static void green_egg_bounce_west(uint16_t m)
{
    MEM8(m+5) &= 0x7F; // face left
    if (move_monster_W(m)) return;
    green_egg_bounce_east(m); // loc_A557
...
static void green_egg_bounce_east(uint16_t m)
{
    MEM8(m+5) |= 0x80; // face right
    if (move_monster_E(m)) return;
    MEM8(m+5) &= 0x7F; // face left
...
static void green_egg_linked_move(uint16_t m)
{
    uint8_t a = MEM8(m+6); // .anim_counter
    uint8_t low = (a + 1) & 7;
    uint8_t high = a & 0xF0;
    ...
    if (MEM8(m+5) & 0x80) { // .ai_flags: facing right
        ...
        if (x < 0) { green_egg_fall_and_patrol(m); return; }
        if ((uint8_t)x < 3) { green_egg_fall_and_patrol(m); return; }
        if (green_egg_check_room(prox, 0x27)) { green_egg_fall_and_patrol(m); return; } // blocked
        green_egg_grow_right(m, partner, prox); // loc_A618
    } else { // facing left
        int8_t x = (int8_t)MEM8(m+3);
        if (x < 0) { green_egg_fall_and_patrol(m); return; }
        if ((uint8_t)x >= 0x21) { green_egg_fall_and_patrol(m); return; }
        if (green_egg_check_room(prox, 0x23)) { green_egg_fall_and_patrol(m); return; } // blocked
        green_egg_grow_left(m, partner, prox); // loc_A5C1
    }
}
...
static int green_egg_check_room(uint16_t prox, uint16_t offset)
{
...
static void green_egg_grow_left(uint16_t m, uint16_t partner, uint16_t prox)
{
    uint16_t map_addr = prox + 2;
    uint8_t layer_val = MEM8(map_addr); // background tile being covered
    MEM8(map_addr) = MEM8(m+0xA) | 0x80; // stamp the partner entity id into the proximity map
    ...
    green_egg_finish_link(m, partner, layer_val);
}

/* loc_A618: mirror of green_egg_grow_left, growing to the right instead
 * (used while facing right). */
static void green_egg_grow_right(uint16_t m, uint16_t partner, uint16_t prox)
{
    uint16_t map_addr = prox - 2;
...
/* loc_A64C: shared tail of green_egg_grow_left/right - syncs the partner's
...
static void green_egg_finish_link(uint16_t m, uint16_t partner, uint8_t saved_layer_val)
{
    MEM8(partner+2) = MEM8(m+2); // .currY

    uint8_t idx = MEM8(m+0xA); // partner entity id
 ...
 */
static void icicle_ai(uint16_t m) // loc_A6B1, shared by monster.flags values 2 and 3
{
    MEM8(m+4) |= 0x20; // .flags

    if (MEM8(m+9) & 1) { // .ai_state: already awake
        icicle_active(m); // loc_A6DB
        return;
    }
...
static void icicle_active(uint16_t m)
{
    if (move_monster_S(m)) return; // still falling
 ...
 */
static void arrow_ai(uint16_t m) // loc_A6F0
{
    ...
    uint8_t used = arrow_try_directions(m); // sub_A71C
    if (used == 3) {
        arrow_try_directions(m); // original tail-calls itself again here, result discarded
    }
}

/* sub_A71C: looks up this monster's ai_state row in arrow_table_left/
...
static uint8_t arrow_try_directions(uint16_t m)
{
    const uint8_t *table = (MEM8(m+5) & 0x80) ? arrow_table_right : arrow_table_left; // .ai_flags
