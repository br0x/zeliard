...
function type0Ai(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 8) === 0) memWrite8(g, m + 8, 0x64);
    ...
    if ((memRead8(g, m + 9) & 1) === 0) {
        ...
        const old = memRead8(g, m + 6);
        memWrite8(g, m + 6, (old + 0x80) & 0xff);
...
function type0AnimStep(g: Uint8Array, m: number): void {
    ...
    if (memRead8(g, m + 6) >= 6) memWrite8(g, m + 6, 0);
}
...
function type0SyncTwin(g: Uint8Array, m: number): void {
    ...
    memWrite8(g, m + 0x15, ((memRead8(g, m + 0x15) & 0x7f) | facing) & 0xff);
}
...
function type2Ai(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 8) === 0) memWrite8(g, m + 8, 0x30);
    ...
    if ((memRead8(g, m + 9) & 1) === 0) {
        ...
        if ((memRead8(g, m + 5) & 0x80) === 0) {
            ...
            memWrite8(g, m + 5, (memRead8(g, m + 5) | 0x80) & 0xff);
        }
        return;
    }

    memWrite8(g, m + 0x0a, (memRead8(g, m + 0x0a) - 1) & 0xff);
    if ((memRead8(g, m + 0x0a) & 3) === 0) {
        const pr = proximity5(g, m);
        memWrite8(g, m + 9, pr.carry ? 1 : 0);
        if (pr.value !== 0xff) {
            memWrite8(g, m + 5, ((memRead8(g, m + 5) & 0x7f) | pr.value) & 0xff);
...
function type3UpdateFacingAndMaybeFire(g: Uint8Array, m: number): void {
    ...
    if ((getRandom(g) & 7) === 0) {
        memWrite8(g, m + 9, (memRead8(g, m + 9) | 4) & 0xff);
        memWrite8(g, m + 0x0a, 0);
    }
}

function type3AnimStep(g: Uint8Array, m: number): number {
    let al = (memRead8(g, m + 6) + 1) & 0xff;
    if (al >= 3) al = 0;
    memWrite8(g, m + 6, al);
    return al;
}
...
function type3FireTick(g: Uint8Array, m: number): void {
    memWrite8(g, m + 6, 3);
    memWrite8(g, m + 0x0a, (memRead8(g, m + 0x0a) + 1) & 0xff);
    if (memRead8(g, m + 0x0a) !== 3) return;

    memWrite8(g, m + 6, 4);

    const x = memRead8(g, m + 3);
    const y = memRead8(g, m + 2) & 0x3f;
    ...
    addProjectileToArray(
        g,
        (memRead8(g, m + 5) & 0x80) !== 0 ? type3ShotRight : type3ShotLeft,
    );

    memWrite8(g, m + 9, memRead8(g, m + 9) & 0xfb);
    memWrite8(g, m + 9, (memRead8(g, m + 9) | 2) & 0xff);
    memWrite8(g, m + 0x0a, 0);
}
...
function type4Ai(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 8) === 0) memWrite8(g, m + 8, 0x60);
    ...
    const oldTimer = memRead8(g, m + 0x0a);
    memWrite8(g, m + 0x0a, (oldTimer + 0x80) & 0xff);
    ...
    if (!pr.carry) {
        if ((memRead8(g, m + 9) & 0x70) === 0) {
            if (pr.value === 0xff) {
                ...
                memWrite8(g, m + 5, ((memRead8(g, m + 5) & 0x7f) | facing) & 0xff);
            } else {
                memWrite8(g, m + 5, ((memRead8(g, m + 5) & 0x7f) | pr.value) & 0xff);
            }
        }
    ...
    const table = (memRead8(g, m + 5) & 0x80) !== 0 ? TYPE4_DIR_RIGHT : TYPE4_DIR_LEFT;
...
function proximity8(g: Uint8Array, m: number): ProxResult8 {
    ...
    const al = (0x10 - memRead8(g, m + 3)) & 0xff;
    if (((al << 24) >> 24) >= 0) {
        const ah = al;
        const carry = (memRead8(g, m + 5) & 0x80) !== 0;
