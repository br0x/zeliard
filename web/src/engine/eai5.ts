...
function moveMonsterNweIfOnAirflow(g: Uint8Array, m: number): number {
    const y = memRead8(g, m + 2); // .currY
    const xRel = memRead8(g, m + 3); // .m_x_rel
    let di = coordsToProxAddr(g, xRel, y) + PROX_COLS;
    di = wrapMapFromAbove(di);

    for (let i = 0; i < 2; i++) {
        const tile = memRead8(g, di + i);
...
function manTopAi(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 8) === 0) memWrite8(g, m + 8, 0x18); // .hp

    if ((memRead8(g, m + 5) & 0x20) !== 0) { // this half was hit
        manHitReaction(g, m);
        return;
    }
    ...
    if ((memRead8(g, m + 9) & 1) !== 0) { // attacking
        manAttackStep(g, m);
        return;
    }
...
function checkGroundedOrStepDown(g: Uint8Array, m: number): number {
    if (memRead8(g, m + 3) === 0) return 1; // left edge
    if (memRead8(g, m + 3) === 0x23) return 1; // right edge

    if (checkLedgeBelow(g, m) !== 0) return 1;

    memWrite8(g, m + 2, (memRead8(g, m + 2) + 1) & 0x3f); // .currY++
    memWrite8(g, m + 0x12, (memRead8(g, m + 0x12) + 1) & 0x3f); // twin.currY++
    return 0;
}
...
function moveEastAndSyncTwin(g: Uint8Array, m: number): number {
    ...
    if (newX === memRead16(g, MAP_WIDTH_ADDR)) newX = 0;
    memWrite16(g, m, newX);
...
function moveWestAndSyncTwin(g: Uint8Array, m: number): number {
    ...
    let newX = (memRead16(g, m) - 1) & 0xffff;
    if (newX === 0xffff) newX = (memRead16(g, MAP_WIDTH_ADDR) - 1) & 0xffff;
    memWrite16(g, m, newX);
    memWrite16(g, m + 0x10, newX);
    memWrite8(g, m + 3, (memRead8(g, m + 3) - 1) & 0xff);
    memWrite8(g, m + 0x13, (memRead8(g, m + 0x13) - 1) & 0xff);
...
function manHitReaction(g: Uint8Array, m: number): void {
    const al = ((memRead8(g, m + 5) & 0xbf) | 0x20) & 0xff;
    memWrite8(g, m + 5, al);
...
function manEndOfFrameSync(g: Uint8Array, m: number): void {
    ...
    memWrite8(g, m + 0x15, (facing | (memRead8(g, m + 0x15) & 0x7f)) & 0xff);
}
...
function manMaybeStartAttack(g: Uint8Array, m: number): void {
    ...
    if ((~memRead8(g, m + 6)) & 3) { // only on a specific anim phase
        manPatrolStep(g, m);
        return;
    }
    memWrite8(g, m + 9, memRead8(g, m + 9) | 1); // begin attack
...
function manAlignToCenterColumn(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 3) > 0x10) { // right of center → approach west
...
function manAttackStep(g: Uint8Array, m: number): void {
    ...
    const phase = memRead8(g, m + 6) & 0x0f;
    if (phase === 0x0b) {
        manFireProjectile(g, m); // ends with end-of-frame sync itself
        return;
    }
    if (phase === 0x0c) {
        memWrite8(g, m + 9, memRead8(g, m + 9) & 0xfe); // end the attack
        memWrite8(g, m + 6, 3);
    }
    manEndOfFrameSync(g, m);
}
...
function manFireProjectile(g: Uint8Array, m: number): void {
    ...
    const desc = (memRead8(g, m + 5) & 0x80) !== 0 ? manShotRight : manShotLeft;
...
function redEggAi(g: Uint8Array, m: number): void {
    ...
    if ((memRead8(g, m + 5) & 0x20) !== 0) { // being hit
...
function redEggHitCheck(g: Uint8Array, m: number): void {
    const hitType = memRead8(g, m + 5) & 0x1f;
    if (
        hitType === 4 || hitType === 5 || hitType === 8 ||
        (hitType === 1 && memRead8(g, SWORD_TYPE) === SWORD_ENCHANTMENT)
    ) {
        heroHitsMonster(g, m);
        return;
    }

    memWrite8(g, m + 5, memRead8(g, m + 5) & 0xdf); // clear the hit bit
    if ((memRead8(g, m + 9) & 2) !== 0) { // .ai_state
        redEggMoveAndState(g, m); // loc_A780
        return;
    }
...
function redEggTryTeleportPartner(g: Uint8Array, m: number): void {
    ...
    const ownAddr = coordsToProxAddr(g, memRead8(g, m + 3), memRead8(g, m + 2));
...
function redEggTeleportWest(
    ...
    const signedX = (memRead8(g, m + 3) << 24) >> 24;
    ...
    for (let row = 0; row < 3; row++) {
        if (isBlocking(g, memRead8(g, addr)) !== 0) { redEggMoveAndState(g, m); return; }
    ...
    const oldTile = memRead8(g, addr);
    memWrite8(g, addr, (idx | 0x80) & 0xff);
    memWrite8(g, PROXIMITY_LAYER2 + idx, oldTile);

    let newX = (memRead16(g, m) + 2) & 0xffff;
    if (newX >= memRead16(g, MAP_WIDTH_ADDR)) newX -= memRead16(g, MAP_WIDTH_ADDR);

    memWrite16(g, partner, newX);
    memWrite8(g, partner + 3, (memRead8(g, m + 3) + 2) & 0xff);
...
function redEggTeleportEast(
    ...
    let newX = memRead16(g, m);
    if (newX >= 2) {
        newX -= 2;
    } else {
        newX = newX + memRead16(g, MAP_WIDTH_ADDR) - 2;
    }
...
function redEggFinalizeTeleportedPartner(g: Uint8Array, m: number, partner: number, idx: number): void {
    ...
    memWrite8(g, partner + 4, (memRead8(g, m + 4) | 0x60) & 0xff); // .flags
    memWrite8(g, partner + 5, memRead8(g, m + 5) & 0x80); // just the facing bit
    memWrite8(g, partner + 6, 4); // .anim_counter
    memWrite8(g, partner + 7, memRead8(g, m + 7)); // .state_flags
    memWrite8(g, partner + 8, 0); // .hp
    memWrite8(g, partner + 9, 2); // .ai_state
    memWrite8(g, partner + 10, 0); // .ai_timer

    if (memRead8(g, MONSTER_INDEX_ADDR) < idx) {
        memWrite8(g, m + 9, memRead8(g, m + 9) | 1); // resume movement next frame
    }
}
...
function redEggDiveState(g: Uint8Array, m: number): void {
    ...
    memWrite8(g, m + 4, memRead8(g, m + 4) & 0x9f); // .flags
}

// loc_A7A9
function redEggAfterFallStep(g: Uint8Array, m: number): void {
    const val = (memRead8(g, m + 6) - 0x10) & 0xff;
    memWrite8(g, m + 6, val);
...
function redEggFaceTowardHeroRow(g: Uint8Array, m: number, val: number): void {
    memWrite8(g, m + 6, val | 0x40); // .anim_counter

    const heroY = memRead8(g, HERO_Y);
    if (heroY === memRead8(g, m + 2) || ((heroY + 1) & 0x3f) === memRead8(g, m + 2)) {
...
function redEggAlignedRow(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 3) <= 0x11) { // .m_x_rel
        redEggTryMoveEastThenWest(g, m); // loc_A7EA
        return;
    }
    redEggTryMoveWestThenEast(g, m); // loc_A7DE
...
function eyeballAi(g: Uint8Array, m: number): void {
    ...
    if ((memRead8(g, m + 9) & 4) !== 0) { // charge-dash mode
...
function eyeballWalkCycle(g: Uint8Array, m: number): void {
    let phase = memRead8(g, m + 6) & 7;
    if (phase === 0) memWrite8(g, m + 9, memRead8(g, m + 9) & 0xfe);
    if (phase === 4) memWrite8(g, m + 9, memRead8(g, m + 9) | 1);
...
function eyeballPrewalkTimer(g: Uint8Array, m: number): void {
    ...
    memWrite8(g, m + 10, (memRead8(g, m + 10) + 1) & 0xff); // .ai_timer
    if ((pr.value & 7) === 0) { // 0 or 0x80: close; 0xFF: far
        memWrite8(g, m + 9, memRead8(g, m + 9) | 2);
    }
...
function eyeballSingleStep(g: Uint8Array, m: number): void {
    if ((memRead8(g, m + 5) & 0x80) !== 0) { // facing right
        if (moveMonsterE(g, m) === 0) memWrite8(g, m + 9, 2);
    } else {
        if (moveMonsterW(g, m) === 0) memWrite8(g, m + 9, 2);
    }
}
...
function vistletSearchState(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 3) <= 0x0f || memRead8(g, m + 3) > 0x12) { // .m_x_rel
        vistletCycleAnim(g, m); // loc_A958
        return;
    }
    memWrite8(g, m + 9, memRead8(g, m + 9) | 1); // begin dive
...
function vistletCycleAnim(g: Uint8Array, m: number): void {
    const low = (memRead8(g, m + 6) + 1) & 3;
    memWrite8(g, m + 6, (memRead8(g, m + 6) & 0xf0) | low);
    vistletFlyStep(g, m); // loc_A966
}
...
function vistletAfterFlyThrottle(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 3) > 0x10) { // .m_x_rel
...
function vistletDiveState(g: Uint8Array, m: number): void {
    ...
    if (phase < 5) {
        memWrite8(g, m + 6, (memRead8(g, m + 6) + 1) & 0xff); // full-byte increment, as in C
        return;
    }
...
function vistletDiveAdvance(g: Uint8Array, m: number): void {
    memWrite8(g, m + 6, (memRead8(g, m + 6) + 1) & 7); // .anim_counter
    if (memRead8(g, m + 6) !== 0) return;
    memWrite8(g, m + 9, 2); // dive finished, begin climb
}
...
function vistletClimbNe(g: Uint8Array, m: number): void {
    ...
    memWrite8(g, m + 9, memRead8(g, m + 9) & 0xfd); // both blocked → climb finished
}
