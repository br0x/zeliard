...
function type0TopAi(g: Uint8Array, m: number): void {
    if (memRead8(g, m + 8) === 0) memWrite8(g, m + 8, 0x30); // .hp

    if ((memRead8(g, m + 5) & 0x20) !== 0) { // .ai_flags: this half was hit
        if ((memRead8(g, m + 5) & 0x1f) === 1) {
            type0DeathReaction(g, m); // loc_A4F7
            return;
        }
        memWrite8(g, m + 5, memRead8(g, m + 5) & 0x9f); // clear the hit bits (0x20, 0x40)
    }

    memWrite8(g, m + 0x15, memRead8(g, m + 0x15) & 0xbf); // twin.ai_flags: clear bit 0x40

    if (type0TryFall(g, m) === 0) return; // fell one row this frame, done

    if ((memRead8(g, m + 9) & 1) !== 0) { // .ai_state: charging up to fire
    ...
    if (pr.carry) {
        if ((memRead8(g, m + 0xa) & 0xf0) !== 0) { // .ai_timer: cooldown accumulated
            memWrite8(g, m + 0xa, 0);
            memWrite8(g, m + 6, 0);
            memWrite8(g, m + 9, memRead8(g, m + 9) | 1); // begin charging
            type0EndSync(g, m);
            return;
        }
    }
...
function type0TryFall(g: Uint8Array, m: number): number {
    if (memRead8(g, m + 3) === 0) return 1;     // .m_x_rel == 0 (left edge)
    if (memRead8(g, m + 3) === 0x23) return 1;  // .m_x_rel == 35 (right edge)

    if (type0CheckLedgeBelow(g, m) !== 0) return 1;

    memWrite8(g, m + 2, (memRead8(g, m + 2) + 1) & 0x3f);       // .currY++
    memWrite8(g, m + 0x12, (memRead8(g, m + 0x12) + 1) & 0x3f); // twin.currY++ (si+0x12)
    return 0;
}
...
function type0CheckLedgeBelow(g: Uint8Array, m: number): number {
    let addr = coordsToProxAddr(g, memRead8(g, m + 3), memRead8(g, m + 2)); // .m_x_rel, .currY
    addr += 0x90; // 4 rows down (4 * 36-column proximity map)
    addr = wrapMapFromAbove(addr);

    for (let col = 0; col < 2; col++) {
        if (isBlocking(g, memRead8(g, addr + col)) !== 0) return 1;
    }
    const combined = (memRead8(g, addr) | memRead8(g, addr + 1)) & 0xff;
...
function type0MoveEastAndSyncTwin(g: Uint8Array, m: number): number {
    if (memRead8(g, m + 3) >= 0x22) return 1; // .m_x_rel >= 34

    if (type0CheckWallAheadE(g, m) !== 0) return 1;

    let newX = (memRead16(g, m) + 1) & 0xffff;
    if (newX === memRead16(g, MAP_WIDTH_ADDR)) newX = 0; // wrap
    memWrite16(g, m, newX);      // .currX
    memWrite16(g, m + 0x10, newX); // twin.currX
    memWrite8(g, m + 3, (memRead8(g, m + 3) + 1) & 0xff);       // .m_x_rel
    memWrite8(g, m + 0x13, (memRead8(g, m + 0x13) + 1) & 0xff); // twin.m_x_rel
    return 0;
}
...
function type0MoveWestAndSyncTwin(g: Uint8Array, m: number): number {
    if (memRead8(g, m + 3) < 2) return 1; // .m_x_rel < 2

    if (type0CheckWallAheadW(g, m) !== 0) return 1;

    let newX = (memRead16(g, m) - 1) & 0xffff;
    if (newX === 0xffff) newX = (memRead16(g, MAP_WIDTH_ADDR) - 1) & 0xffff; // wrap
    memWrite16(g, m, newX);
    memWrite16(g, m + 0x10, newX);
...
function type0DeathReaction(g: Uint8Array, m: number): void {
    let al = memRead8(g, m + 5) & 0x1f; // == 1 here
    al = (al & 0xbf) & 0xff;
    al = (al | 0x20) & 0xff;
    memWrite8(g, m + 5, al);              // .ai_flags
    memWrite8(g, m + 0x15, (al | 0x60) & 0xff); // twin.ai_flags
    heroHitsMonster(g, m);
}
...
function type0EndSync(g: Uint8Array, m: number): void {
    memWrite8(g, m + 0x16, memRead8(g, m + 6)); // twin.anim_counter = .anim_counter

    const stateBits = memRead8(g, m + 4) & 0x60;
    memWrite8(g, m + 0x14, ((memRead8(g, m + 0x14) & 0x9f) | stateBits) & 0xff); // twin.flags

    const facing = memRead8(g, m + 5) & 0x80;
    memWrite8(g, m + 0x15, ((memRead8(g, m + 0x15) & 0x7f) | facing) & 0xff); // twin.ai_flags
}
...
function type0WanderStep(g: Uint8Array, m: number): void {
    memWrite8(g, m + 0xa, (memRead8(g, m + 0xa) + 1) & 0xff); // .ai_timer
    memWrite8(g, m + 6, 1);  // .anim_counter
    memWrite8(g, m + 4, (memRead8(g, m + 4) | 0x60) & 0xff); // .flags
    ...
    if ((getRandom(g) & 1) !== 0) {
        ...
        if (type0MoveEastAndSyncTwin(g, m) === 0) { // moved
            memWrite8(g, m + 5, (memRead8(g, m + 5) | 0x80) & 0xff); // face right
        }
    }
    type0EndSync(g, m);
}
...
function type0ChargeTick(g: Uint8Array, m: number): void {
    memWrite8(g, m + 6, (memRead8(g, m + 6) + 1) & 0x0f); // .anim_counter

    if (memRead8(g, m + 6) === 0) {
        memWrite8(g, m + 9, memRead8(g, m + 9) & 0xfe); // .ai_state: end the charge
        memWrite8(g, m + 6, 1);
        memWrite8(g, m + 4, (memRead8(g, m + 4) | 0x60) & 0xff); // .flags
        type0EndSync(g, m);
        return;
    ...
    memWrite8(g, m + 4, memRead8(g, m + 4) & 0x1f); // .flags: clear hit/charge display bits
    if (memRead8(g, m + 6) === 8) {
...
function type0FireProjectile(g: Uint8Array, m: number): void {
    ...
    const y = (memRead8(g, m + 2) + 1) & 0xff;    // .currY + 1
    ...
    const desc = (memRead8(g, m + 5) & 0x80) !== 0 ? type0ShotRight : type0ShotLeft; // .ai_flags
...
function monsterToHeroProximityAndDirection4(g: Uint8Array, m: number): ProxResult {
    ...
    if (memRead8(g, m + 3) < 0x11) { // .m_x_rel: monster left of the hero
        return { value: 0x80, carry: (memRead8(g, m + 5) & 0x80) !== 0 }; // facing right => facing hero
    } else { // right of (or level with) the hero
        return { value: 0x00, carry: (memRead8(g, m + 5) & 0x80) === 0 }; // facing left => facing hero
    }
}
...
function type2Ai(g: Uint8Array, m: number): void {
    ...
    if ((memRead8(g, m + 5) & 0x20) !== 0) { // .ai_flags: was hit
    ...
    if ((memRead8(g, m + 9) & 2) !== 0) { type2StateB(g, m); return; }   // loc_A78D
    if ((memRead8(g, m + 9) & 1) !== 0) { type2StateA(g, m); return; }   // loc_A76E
    if ((memRead8(g, m + 9) & 4) !== 0) { type2StateC(g, m); return; }   // loc_A815
...
function type2StateMain(g: Uint8Array, m: number): void {
    ...
    if (!pr.carry) {
        if ((memRead8(g, m + 9) & 0x70) !== 0) { // .ai_state: throttle running
            type2MoveAndAnimate(g, m); // skip the vertical step
            return;
        }
        if (pr.value === 0xff) {
            const al = ((getRandom(g) << 1) & 0x80) & 0xff;
            memWrite8(g, m + 5, ((memRead8(g, m + 5) & 0x7f) | al) & 0xff); // random facing
        } else {
            memWrite8(g, m + 5, ((memRead8(g, m + 5) & 0x7f) | pr.value) & 0xff); // face the hero
        }
    }
...
function type2VerticalApproach(g: Uint8Array, m: number): void {
    const diff = (memRead8(g, HERO_Y) - memRead8(g, m + 2)) & 0xff; // .currY
    if (((diff << 24) >> 24) >= 0) {
        moveMonsterS(g, m);
    } else {
        moveMonsterN(g, m);
...
function type2MoveAndAnimate(g: Uint8Array, m: number): void {
    ...
    memWrite8(g, m + 9, (memRead8(g, m + 9) + 0x10) & 0xff); // .ai_state throttle

    const idx = ((memRead8(g, m + 9) >> 4) & 7) & 0xff;
    const table = (memRead8(g, m + 5) & 0x80) !== 0 ? TYPE2_DIR_TABLE_RIGHT : TYPE2_DIR_TABLE_LEFT;
...
function type2StateB(g: Uint8Array, m: number): void {
    ...
    if (pr.value === 0xff) {
        const al = ((getRandom(g) << 1) & 0x80) & 0xff;
        memWrite8(g, m + 5, ((memRead8(g, m + 5) & 0x7f) | al) & 0xff);
    } else {
...
function type2MoveAndAnimateB(g: Uint8Array, m: number): void {
    memWrite8(g, m + 6, (((memRead8(g, m + 6) + 1) & 3) | 8) & 0xff); // .anim_counter
    memWrite8(g, m + 9, (memRead8(g, m + 9) + 0x10) & 0xff);           // .ai_state throttle
...
function type2StateBEnd(g: Uint8Array, m: number): void {
    memWrite8(g, m + 6, 0x0c); // .anim_counter
...
function type3Ai(g: Uint8Array, m: number): void {
    ...
    if ((memRead8(g, m + 9) & 1) !== 0) { // .ai_state: mid-jump
...
function type3PatrolStep(g: Uint8Array, m: number): void {
    memWrite8(g, m + 6, (memRead8(g, m + 6) + 1) & 0xf3); // .anim_counter
...
function type3StateJump(g: Uint8Array, m: number): void {
    if ((memRead8(g, m + 9) & 2) !== 0) { // .ai_state: recovering
    ...
    memWrite8(g, m + 6, 4); // .anim_counter
...
function type3CheckVerticalBlock(g: Uint8Array, m: number): number {
    if ((memRead8(g, m + 9) & 4) === 0) { // .ai_state
...
function type3FlipAndRecover(g: Uint8Array, m: number): void {
    memWrite8(g, m + 9, memRead8(g, m + 9) | 2);     // .ai_state
    memWrite8(g, m + 5, memRead8(g, m + 5) ^ 0x80);  // .ai_flags: flip facing
...
function type4Ai(g: Uint8Array, m: number): void {
    memWrite8(g, m + 4, (memRead8(g, m + 4) | 0x20) & 0xff); // .flags: force "hit" bit every frame

    if ((memRead8(g, m + 9) & 2) !== 0) { // .ai_state: falling
        type4FallThrottle(g, m); // loc_A9B4
        return;
    }
    if ((memRead8(g, m + 9) & 1) !== 0) { // .ai_state: about to fall
        type4StartFall(g, m); // loc_A98C
        return;
    }

    if (memRead8(g, m + 3) < 8) return;     // .m_x_rel: hero not under it yet
    if (memRead8(g, m + 3) >= 0x13) return; // .m_x_rel: hero has passed it

    if ((getRandom(g) & 3) === 0) {
        memWrite8(g, m + 9, memRead8(g, m + 9) | 1); // arm the drop
    }
}
...
function type4StartFall(g: Uint8Array, m: number): void {
    ...
    const topRow = (memRead8(g, VIEWPORT_TOP_ROW) - 1) & 0xff;
    const rel = (memRead8(g, m + 2) - topRow) & 0x3f; // .currY
    if (rel < 0x13) {
        memWrite8(g, SOUND_FX_REQUEST, 0x21);
    }
}
...
function type4FallThrottle(g: Uint8Array, m: number): void {
    ...
    memWrite8(g, m + 7, ((memRead8(g, m + 7) & 0xf0) | 1) & 0xff);
