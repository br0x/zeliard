...
export function findMonstersNearHero(
    ...
    outPartner.v = g16(g, MONSTERS_LIST);
    outIdx.v = 0;
    for (;;) {
        const entryX = g16(g, outPartner.v);
        if (entryX === 0xffff) return 1; // .currX terminator

        if (g16(g, outPartner.v + 11) !== 0xffff) { // .spwnX set
            // keep scanning
        } else if (g8(g, outPartner.v + 1) === 0xff) { // currX high byte
            // loc_98F4 equivalent
            if (g8(g, outPartner.v + 2) !== 0x7f) {
                return 0;
            }
        } else {
            const win = isInProximityWindow(g, entryX);
            if (!win.inside) {
                if ((g8(g, outPartner.v + 4) & 0x10) === 0) {
                    return 0;
                }
            }
        }
...
function turtleAi(g: Uint8Array, m: number): void { // loc_A281
    ...
    if ((g8(g, m + 5) & 0x20) !== 0) { // hit
        heroHitsMonster(g, m);
        return;
    }

    if ((g8(g, m + 9) & 8) !== 0) { turtleSettle(g, m); return; }
    if ((g8(g, m + 9) & 4) !== 0) { turtleBounceDash(g, m); return; }
...
function turtleLanded(g: Uint8Array, m: number): void {
    if ((g8(g, m + 9) & 1) !== 0) { turtleSnapForward(g, m); return; }
    if ((g8(g, m + 9) & 2) !== 0) { turtleSnapBackward(g, m); return; }
...
function turtlePatrol(g: Uint8Array, m: number): void {
    ...
    if ((g8(g, m + 5) & 0x80) !== 0) turtleGoEast(g, m);
...
function turtleAligned(g: Uint8Array, m: number): void {
    if ((getRandom(g) & 3) === 0) s8(g, m + 9, 5); // .ai_state
    if (g8(g, m + 3) < 0x11) turtleGoEast(g, m);
    else turtleGoWest(g, m);
}
...
function turtleSnapForward(g: Uint8Array, m: number): void {
    let low = g8(g, m + 6) & 0x0f; // .anim_counter low nibble
    if (low < 8) {
        s8(g, m + 6, 8);
        return;
    }
...
function turtleBounceDash(g: Uint8Array, m: number): void {
    let v = (g8(g, m + 6) & 0x0f) + 1; // .anim_counter
    if (v < 0x0f) {
        s8(g, m + 6, v);
        return;
    }
...
function turtleSettle(g: Uint8Array, m: number): void {
    let low = (g8(g, m + 6) + 1) & 0x0f; // .anim_counter
    if (low >= 0x0d) low = 0x0b;
    s8(g, m + 6, low);

    if ((g8(g, m + 0x0a) & 1) !== 0) { // initial delay already elapsed
        turtleSettleStep2(g, m); // loc_A3F8
        return;
    }

    moveMonsterS(g, m);
    s8(g, m + 9, (g8(g, m + 9) + 0x10) & 0xff); // high-nibble delay counter
    if ((g8(g, m + 9) & 0xf0) !== 0) return; // still counting
    s8(g, m + 0x0a, g8(g, m + 0x0a) | 1); // delay elapsed
}

// loc_A3F8: one plain E/W step before switching to trajectory drift.
function turtleSettleStep2(g: Uint8Array, m: number): void {
    if ((g8(g, m + 0x0a) & 4) !== 0) { // .ai_timer
        turtleSettleTrajectory(g, m); // loc_A412
        return;
    }
    s8(g, m + 0x0a, g8(g, m + 0x0a) | 4);
    if ((g8(g, m + 0x0a) & 8) !== 0) {
        moveMonsterW(g, m);
    } else {
        moveMonsterE(g, m);
...
function turtleSettleTrajectory(g: Uint8Array, m: number): void {
    const traj = (g8(g, m + 5) & 0x80) !== 0 ? TRAJECTORY_RIGHT : TRAJECTORY_LEFT;

    let al = g8(g, m + 9); // .ai_state
    al = ((al << 3) | (al >>> 5)) & 0xff; // rol al,1 (×3)
    const idx = al & 7;

    s8(g, m + 9, (g8(g, m + 9) + 0x20) & 0xff); // .ai_state
    if ((g8(g, m + 9) & 0xe0) === 0) {
        s8(g, m + 0x0a, 0); // .ai_timer
        s8(g, m + 9, 2); // .ai_state
    }

    if (monsterMoveInDirection(g, m, traj[idx] ?? 0) !== 0) return;

    const hi = g8(g, m + 9) & 0xe0; // .ai_state
    if (hi !== 0 && hi < 0xc0) {
        s8(g, m + 5, g8(g, m + 5) ^ 0x80); // flip facing
    }
}
...
function greenEggAi(g: Uint8Array, m: number): void { // loc_A466
    ...
    if ((g8(g, m + 5) & 0x20) === 0) { // not hit
        greenEggCommon(g, m); // loc_A4EC
        return;
    }

    const al = g8(g, m + 5) & 0x1f; // hit-type code
    if (al === 4 || al === 5 || al === 8) {
        heroHitsMonster(g, m);
        return;
    }
    if (al === 1 && g8(g, SWORD_TYPE) === SWORD_ENCHANTMENT) {
        heroHitsMonster(g, m);
        return;
    }
    if ((g8(g, m + 6) & 1) !== 0) { // odd frame
        heroHitsMonster(g, m);
        return;
    }

    // loc_A4B1: hit absorbed — clear the hit flag and try to link up.
    s8(g, m + 5, g8(g, m + 5) & 0xdf); // .ai_flags
    if ((g8(g, m + 7) & 0x40) === 0) { // not already linked
        const partner = { v: 0 };
        const idx = { v: 0 };
        if (findMonstersNearHero(g, m, partner, idx) === 0) {
            s16(g, partner.v, 0xff00);
            if ((g8(g, partner.v + 7) & 0x40) !== 0) {
                s8(g, partner.v + 7, g8(g, partner.v + 7) & 0xbf);
                const otherIdx = g8(g, partner.v + 0x0a);
                const other = (g16(g, MONSTERS_LIST) + otherIdx * 0x10) & 0xffff;
                s8(g, other + 2, 0);
            }
            s8(g, partner.v + 2, 0x7f);
            s8(g, m + 0x0a, idx.v);
...
function greenEggCommon(g: Uint8Array, m: number): void {
    const bitSet = (g8(g, m + 9) & 1) !== 0; // .ai_state
...
function greenEggFallAndPatrol(g: Uint8Array, m: number): void {
    ...
    s8(g, m + 6, (g8(g, m + 6) - 0x10) & 0xff); // landing countdown
    if ((g8(g, m + 6) & 0xf0) !== 0) return;

    s8(g, m + 6, g8(g, m + 6) | 0x40); // landed

    const heroY = g8(g, HERO_Y);
    const myY = g8(g, m + 2); // .currY
    const aligned = heroY === myY || ((heroY + 1) & 0x3f) === myY;

    if (!aligned) {
        if ((g8(g, m + 5) & 0x80) !== 0) greenEggBounceEast(g, m);
        else greenEggBounceWest(g, m);
        return;
    }

    // aligned — head back towards the anchor column
    if (g8(g, m + 3) <= 0x10) greenEggBounceEast(g, m);
    else greenEggBounceWest(g, m);
}
...
function greenEggLinkedMove(g: Uint8Array, m: number): void {
    ...
    const partnerIdx = g8(g, m + 0x0a);
    const partner = (g16(g, MONSTERS_LIST) + partnerIdx * 0x10) & 0xffff;
    const prox = coordsToProxAddr(g, g8(g, m + 3), g8(g, m + 2));

    if ((g8(g, m + 5) & 0x80) !== 0) { // facing right
        const x = (g8(g, m + 3) << 24) >> 24; // int8_t
...
function greenEggCheckRoom(g: Uint8Array, prox: number, offset: number): number {
    ...
    for (let row = 0; row < 3; row++) {
        ...
        if (isBlocking(g, g8(g, addr + 1)) !== 0) return 1;
        if (isBlocking(g, g8(g, addr + 2)) !== 0) return 1;
        addr += 0x24; // PROX_COLS
...
function greenEggGrowLeft(g: Uint8Array, m: number, partner: number, prox: number): void {
    ...
    const layerVal = g8(g, mapAddr); // covered background tile
    s8(g, mapAddr, (g8(g, m + 0x0a) | 0x80) & 0xff); // stamp partner entity id

    s8(g, partner + 4, g8(g, m + 4) & 0x1f); // inherit species sub-type

    let x = (g16(g, m) + 2) & 0xffff;
    const mapWidth = g16(g, 0xc002); // ADDR_MAP_WIDTH word
    ...
    s8(g, partner + 3, (g8(g, m + 3) + 2) & 0xff); // .m_x_rel

    s8(g, m + 6, 0x16); // this half's sprite
    s8(g, partner + 6, 0x17); // partner sprite
...
function greenEggGrowRight(g: Uint8Array, m: number, partner: number, prox: number): void {
    const mapAddr = prox - 2;
    const layerVal = g8(g, mapAddr);
    s8(g, mapAddr, (g8(g, m + 0x0a) | 0x80) & 0xff);

    s8(g, partner + 4, g8(g, m + 4) & 0x1f);

    let x = ((g16(g, m) - 2) << 16) >> 16; // int16_t
    if (x < 0) {
        x += g16(g, 0xc002) /* MAP_WIDTH */ << 16 >> 16;
    }
    s16(g, partner, x);
...
function greenEggFinishLink(g: Uint8Array, m: number, partner: number, savedLayerVal: number): void {
    ...
    const idx = g8(g, m + 0x0a); // partner entity id
    s8(g, PROXIMITY_LAYER2 + idx, savedLayerVal);

    s8(g, partner + 7, 0);
    s8(g, partner + 8, 0);
    s8(g, partner + 9, 0);
    s8(g, m + 7, g8(g, m + 7) & 0xbf); // clear "growth in progress"

    if (g8(g, MONSTER_INDEX) < g8(g, m + 0x0a)) {
        s8(g, partner + 9, g8(g, partner + 9) | 1); // process later this frame
    }
}
...
function icicleAi(g: Uint8Array, m: number): void { // loc_A6B1
    s8(g, m + 4, g8(g, m + 4) | 0x20); // .flags

    if ((g8(g, m + 9) & 1) !== 0) { // already awake
...
function arrowTryDirections(g: Uint8Array, m: number): number {
    const table = (g8(g, m + 5) & 0x80) !== 0 ? ARROW_TABLE_RIGHT : ARROW_TABLE_LEFT;
    const rowBase = (g8(g, m + 9) ?? 0) * 15; // .ai_state
    ...
    for (let remaining = 5; remaining > 0; remaining--) {
        ...
        if (monsterMoveInDirection(g, m, dir) !== 0) {
            s8(g, m + 9, table[entryIdx + 1] ?? 0); // .ai_state
            s8(g, m + 6, table[entryIdx + 2] ?? 0); // .anim_counter
            return remaining;
        }
    }

    s8(g, m + 5, g8(g, m + 5) ^ 0x80); // flip facing, nothing worked
    return 0;
}
