...
import { memRead8, memRead16, memWrite8 } from '../core/ts-memory.js';
...
export function dispatchAirflows(g: Uint8Array, si: number): void {
    ...
    switch (dir) {
        case 0: // up
            moveHeroUp(g);
            moveHeroUp(g);
            memWrite8(g, AIR_UP_TILE_FOUND, 0xff);
            memWrite8(g, JUMP_PHASE_FLAGS, 0); // no jumps while flying
            memWrite8(g, HERO_ANIM_PHASE, 0x80); // idle
            break;
        case 1: // left
...
export function checkAirflowsOnHero(g: Uint8Array): void {
    memWrite8(g, AIR_UP_TILE_FOUND, 0);
...
export function updateBossHeartbeatVolume(g: Uint8Array): void {
    ...
    if ((memRead16(g, TEAR_X) & 0xffff) === 0xffff) {
    ...
    let rel: number;
    {
        const tearX = memRead16(g, TEAR_X);
        ...
        if (tearX >= proxLeft) {
            ...
            rel = (memRead16(g, MAP_WIDTH) - proxLeft + tearX) & 0xffff;
        }
    }
    if (rel > 35) {
        memWrite8(g, HEARTBEAT_VOLUME, 0);
    ...
    memWrite8(g, HEARTBEAT_VOLUME, HEARTBEAT_DISTANCE_ATTENUATION[dist2] ?? 0);
}
