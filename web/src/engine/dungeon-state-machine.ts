...
export function clearHeroInViewport(g: Uint8Array): void {
    let di =
        0xe900 +
        (memRead8(g, HERO_HEAD_Y_VIEW) ?? 0) * 28 +
...
export function rokaRun(g: Uint8Array): void {
    ...
    memWrite8(g, FRAME_TIMER, 0); // ADDR_FRAME_TIMER (0xFF1A)
    memWrite8(g, DUNGEON_STATE, DUNGEON_STATE_ROKA_RUN);
    if ((memRead8(g, LEFT_RUN) & 1) !== 0) {
...
export function afterRunAnimation(
    ...
    if ((memRead8(g, PLACE_MAP_ID) & 0x80) !== 0) {
        // town — signal game.js to init town entry
        memWrite8(g, 0xffe2 /* DUNGEON_EXIT_FLAG */, 0xff);
        memWrite8(g, DUNGEON_STATE, DUNGEON_STATE_EXIT);
        return;
    }
    ...
    memWrite8(g, 0xff34 /* IS_BOSS_CAVERN */, (mdtDesc0 & 0x80) !== 0 ? 0xff : 0);
    memWrite8(g, IS_JASHIIN_CAVERN, (mdtDesc0 & 0x40) !== 0 ? 0xff : 0);
    memWrite8(g, 0xff2e /* BOSS_BEING_HIT */, 0);
    memWrite8(g, 0xff2f /* SPRITE_FLASH_FLAG */, 0);
    // Clear_Viewport_proc(): stub — JS clears the canvas

    memWrite8(g, HERO_XV, 12);
    const heroHeadY = memRead8(g, HERO_Y_VIEW_INIT);
    memWrite8(g, HERO_HEAD_Y_VIEW, heroHeadY);
    memWrite8(g, BYTE_9F00, heroHeadY);
    ...
    if (!statics.isFromTown) {
        memWrite8(
            g,
            VIEWPORT_TOP_ROW,
            (memRead8(g, VIEWPORT_TOP_ROW) + statics.savedYViewInit - heroHeadY) & 0x3f,
        );
    }
...
export function cavernGameInit(
    ...
    memWrite8(g, 0x9f20 /* SLIDE_TICKS_REMAINING */, 0);
    memWrite8(g, 0x9f21 /* HORIZ_MOVEMENT_ACCUM */, 0);
    memWrite8(g, 0x9f22 /* SLIDE_DIRECTION */, 0);
    memWrite8(g, PROJECTILES_LIST, 0xff);
    memWrite8(g, 0xeda0 /* BOSS_EXPLOSIONS_LIST */, 0xff);
    memWrite16(g, MAGIC_PROJECTILES, 0xffff);
    memWrite8(g, 0xff2e /* BOSS_BEING_HIT */, 0);
    memWrite8(g, 0xff2f /* SPRITE_FLASH_FLAG */, 0);
    memWrite8(g, 0xff30 /* BOSS_IS_DEAD */, 0);
    memWrite8(g, 0x9f01 /* BOSS_PLACEMENT */, 0);

    if (memRead8(g, 0xff34 /* IS_BOSS_CAVERN */) !== 0) {
        // render_hud_bars_with_enemy(): body fully commented out → no-op
        memWrite8(g, 0x9f02 /* BYTE_9F02 */, 0xff);
        // Render_Animated_Tile_Strip_proc / Update_Local_Attribute_Cache /
        // Copy_Tile_Buffer_To_VRAM: stubs
        memWrite8(g, 0xff37 /* HERO_SPRITE_HIDDEN */, 0);
        clearHeroInViewport(g);
        memWrite8(g, 0x9f02, 0);

        // Override enp_grp_idx with boss_grp from the mdt descriptor
        const si = memRead16(g, MDT);
        const bossGrp = memRead8(g, si + 5);
        memWrite8(g, si + 4, bossGrp);

        // Jashiin room 2 (mpa0) enters straight from the room-1 cutscene:
        // fight starts immediately, no encounter flash.
        if ((memRead8(g, PLACE_MAP_ID) & 0x7f) === 30) {
            ...
            const bossStatePtr = memRead16(g, 0xa002 /* ADDR_BOSS_STATE_PTR */);
            memWrite8(g, 0x9f01 /* BOSS_PLACEMENT */, memRead8(g, bossStatePtr + 8));
            memWrite8(g, 0xffa0 /* BOSS_MODE */, 0xff);
            memWrite8(g, 0xff9f /* BOSS_HEALTH_REQUEST */, 0xff);
            memWrite8(g, MAO2_START_LATCH, 0xff);
            memWrite8(g, DUNGEON_STATE, DUNGEON_STATE_NORMAL);
        } else {
            memWrite8(g, DUNGEON_STATE, DUNGEON_STATE_BOSS_ENCOUNTER);
        }
    } else {
        // regular cavern path
        memWrite8(g, BOSS_MODE, 0);
        // Print_Gold_Decimal()
        memWrite8(g, GOLD_RENDER_REQUEST, 0xff);
    }

    if (
        memRead8(g, IS_JASHIIN_CAVERN) !== 0 &&
        (memRead8(g, PLACE_MAP_ID) & 0x7f) === 29
    ) {
        // Jashiin room 1 (mp90): cutscene only
        memWrite8(g, 0x9f26, 0xff);
        memWrite16(g, LEFT_COL_NUM, 41);
        memWrite8(g, HERO_XV, 5);
        unpackMap(g);
        clearViewportBuffer(g);
        memWrite8(g, DUNGEON_STATE, DUNGEON_STATE_JASHIIN_CUTSCENE);
    } else {
        // non-Jashiin cavern startup
        unpackMap(g);
        if (memRead8(g, 0x9f27) !== 0) {
            clearViewportBuffer(g);
            if (mainUpdateRender) mainUpdateRender(g);
            memWrite8(g, 0x9f26, 0);
        } else {
        ...
        // loc_6266
        if (memRead8(g, DEATH_ALREADY_PROCESSED) !== 0) {
            processHeroDeathHook?.(g);
            return;
        }
        // not_dead
        if (memRead8(g, BYTE_9F02) !== 0) {
            memWrite8(g, BYTE_9F02, 0);
...
export function dungeonUpdateRokaRun(
    ...
    if ((memRead8(g, FRAME_TIMER) & 15) !== 0) return;
    const phase = memRead8(g, ROKA_PHASE);
    if (phase >= 25) {
        afterRunAnimation(g, statics);
        if (memRead8(g, DUNGEON_STATE) === DUNGEON_STATE_ROKA_RUN) {
            memWrite8(g, DUNGEON_STATE, DUNGEON_STATE_NORMAL);
        }
    ...
    memWrite8(g, ROKA_PHASE, (phase + 1) & 0xff);
...
export function dungeonUpdate(
    ...
    switch (memRead8(g, DUNGEON_STATE)) {
