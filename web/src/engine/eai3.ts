...
function type0TurnAround(g: Uint8Array, m: number): void {
    const timer = g8(g, m + 0x0a); // .ai_timer
    s8(g, m + 0x0a, 0);
...
function type0State0(g: Uint8Array, m: number): void {
    ...
    if ((g8(g, m + 6) & 1) === 0) return; // only act on odd frames
...
function type0State3(g: Uint8Array, m: number): void {
    if (g8(g, m + 0x0a) === 1) { // .ai_timer
        s8(g, m + 9, 4); // .ai_state
        s8(g, m + 0x0a, 0xff);
    }
    s8(g, m + 6, 0x0b); // .anim_counter

    if ((g8(g, m + 5) & 0x80) === 0) { // facing left
        s8(g, m + 0x0a, (g8(g, m + 0x0a) + 1) & 0xff);
        if (moveMonsterNW(g, m) === 0) s8(g, m + 5, g8(g, m + 5) ^ 0x80);
    } else { // facing right
        s8(g, m + 0x0a, (g8(g, m + 0x0a) + 1) & 0xff);
        if (moveMonsterNE(g, m) === 0) s8(g, m + 5, g8(g, m + 5) ^ 0x80);
    }
}

// loc_A3AC: continue the dodge with a plain W/E step.
function type0State4(g: Uint8Array, m: number): void {
    if (g8(g, m + 0x0a) === 1) s8(g, m + 9, 5); // .ai_timer / .ai_state

    s8(g, m + 6, 8); // .anim_counter

    if ((g8(g, m + 5) & 0x80) === 0) { // facing left
        s8(g, m + 0x0a, (g8(g, m + 0x0a) + 1) & 0xff);
        if (moveMonsterW(g, m) === 0) s8(g, m + 5, g8(g, m + 5) ^ 0x80);
    } else { // facing right
        s8(g, m + 0x0a, (g8(g, m + 0x0a) + 1) & 0xff);
        if (moveMonsterE(g, m) === 0) s8(g, m + 5, g8(g, m + 5) ^ 0x80);
    }
}
...
function type0State7(g: Uint8Array, m: number): void {
    ...
    if ((g8(g, m + 5) & 0x80) === 0) { // facing left
        ...
        if (checkCollisionN2(g, m) !== 0) {
            ...
            s8(g, m + 5, g8(g, m + 5) ^ 0x80); // keep climbing, flip facing
        }
    } else { // facing right
...
function type1Ai(g: Uint8Array, m: number): void { // loc_A44D
    ...
    if ((g8(g, m + 9) & 8) === 0) { // not yet in the crawl cycle
        if ((g8(g, m + 9) & 4) === 0) { // pick an initial facing
            s8(g, m + 5, g8(g, m + 5) | 0x80); // tentatively face right
            if (g8(g, m + 3) >= 0x11) { // too far right of the anchor
                s8(g, m + 5, g8(g, m + 5) ^ 0x80); // face left instead
            }
        }
        ...
        const sum = (g8(g, m + 6) & 0xf0) + 0x80;
        s8(g, m + 6, sum & 0xff);
        if (sum >= 0x100) {
            s8(g, m + 6, 0);
            s8(g, m + 9, g8(g, m + 9) | 8); // enter the crawl cycle
        }
        return;
    }
    ...
    const oldAnim = g8(g, m + 6); // pre-increment value used as table index
    ...
    const dirTable = (g8(g, m + 5) & 0x80) !== 0 ? TYPE1_DIR_TABLE_RIGHT : TYPE1_DIR_TABLE_LEFT;
    const dir = dirTable[oldAnim] ?? 0;
    if (monsterMoveInDirection(g, m, dir) !== 0) return; // moved

    s8(g, m + 9, g8(g, m + 9) & 0xf7); // .ai_state
    if (g8(g, m + 6) === 1) {
        s8(g, m + 9, g8(g, m + 9) | 4); // .ai_state
        s8(g, m + 5, g8(g, m + 5) ^ 0x80); // flip facing
    }
    s8(g, m + 6, 0);
...
function type2State0(g: Uint8Array, m: number): void {
    ...
    s8(g, m + 0x0a, (g8(g, m + 0x0a) + 1) & 0xff); // .ai_timer
    if (g8(g, m + 0x0a) >= 7) {
        s8(g, m + 9, 1); // .ai_state
...
function type2State2(g: Uint8Array, m: number): void {
    if ((g8(g, m + 9) & 0x80) !== 0) { // already committed to recovery
        type2EnterRecovery(g, m);
        return;
    }

    const sum = g8(g, m + 6) + 0x40;
    s8(g, m + 6, sum & 0xff);
    if (sum < 0x100) return; // throttling
...
function type2Fire(g: Uint8Array, m: number): void {
    s8(g, m + 6, 6); // .anim_counter
    s8(g, m + 9, g8(g, m + 9) | 0x80); // .ai_state

    const x = g8(g, m + 3); // .m_x_rel
    type2ShotLeft[0] = x;
    type2ShotRight[0] = (x + 1) & 0xff;

    const y = g8(g, m + 2) & 0x3f; // .currY
    type2ShotLeft[1] = y;
    type2ShotRight[1] = y;

    const desc = (g8(g, m + 5) & 0x80) !== 0 ? type2ShotRight : type2ShotLeft;
...
function type2ProxCheck(g: Uint8Array, m: number): ProxResult {
    ...
    if (g8(g, m + 3) < 0x11) { // left of the hero
...
function type3Ai(g: Uint8Array, m: number): void { // loc_A66E
    ...
    const oldAnim = g8(g, m + 6); // saved across the fall attempt
    s8(g, m + 6, 0);
    if (moveMonsterS(g, m) !== 0) return; // still falling

    s8(g, m + 6, oldAnim); // restore

    if ((g8(g, m + 9) & 1) !== 0) { // already engaged/patrolling
    ...
    if (dr.carry) {
        if (dr.dist < 0x0a) {
            s8(g, m + 9, g8(g, m + 9) | 1); // begin patrol/engage
        }
        return;
    }
    if (dr.value === 0xff) return;
    s8(g, m + 5, (g8(g, m + 5) & 0x7f) | dr.value); // face the hero
}
...
function type3Patrol(g: Uint8Array, m: number): void {
    s8(g, m + 0x0a, (g8(g, m + 0x0a) + 1) & 0xff); // .ai_timer
    if (g8(g, m + 0x0a) === 0x14) {
        s8(g, m + 9, g8(g, m + 9) & 0xfe); // stop patrolling
        return;
    }
    ...
    if ((g8(g, m + 5) & 0x80) === 0) { // facing left
        if (moveMonsterW(g, m) === 0) {
            if (moveMonsterNW(g, m) === 0) {
                s8(g, m + 9, g8(g, m + 9) & 0xfe); // escaped
                return;
            }
        }
    } else { // facing right
...
function type3ProxCheck(g: Uint8Array, m: number): DistResult {
    ...
    if (g8(g, m + 3) <= 0x11) { // at/left of the anchor column
        const dx = (0x11 - g8(g, m + 3)) & 0xff;
        const facingRight = (g8(g, m + 5) & 0x80) !== 0;
        return { value: 0x80, dist: dx, carry: facingRight };
    } else { // right of the anchor column
        const dx = (g8(g, m + 3) - 0x11) & 0xff;
        const facingLeft = (g8(g, m + 5) & 0x80) === 0;
