...
- **Save-compat fix (latest)**: `TEAR_FLAGS` table (cavern order), `countCollectedTears()`, `getTearCount() = min(9, max(readU8(0xA0), flags))`. `syncTearOverlay()` and `startRokademo()` use `getTearCount()`; `performGameRestore` writes the derived count back to `ADDR_TEAR_COUNT` after `loadSaveState` so demo slot selection and saved data stay consistent. Old saves (counter 0, flags set) now render correctly; new saves (counter incremented) still work via max(); the 9th (Jashiin) tear falls back to the counter / byte 0x47.
