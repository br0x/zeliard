# TypeScript Migration History

This is the archived Stage 0-10 migration diary. Statements about "current
state", wasm, bridge code, dispatch, C sources, and parity oracles reflect the
stage being documented, not the current runtime. For the live status and active
goals, see [../MIGRATION_PLAN.md](../MIGRATION_PLAN.md).
...
| Scenes | `opening-intro.js`, `ending-demo.js`, `indoor-*.js` ×8, `inventory-screen.js` | Already class-based ES modules |
...
12. ✅ `config/engine.ts` + `data/assets.ts` — remaining static config moved
...
13. ✅ Conversation leftovers — `readNpcConversationBytes` (the NPC pointer-
...
14. ✅ `core/roka-demo.ts` — the post-boss Tear of Esmesanti collection demo:
...
15. ✅ `core/transitions.ts` — pure transition helpers extracted from the
...
16. ✅ `input/key-router.ts` — the keyboard dispatch chain: `KeyRouter`
...
17. ✅ `render/` foundations — `render/canvas.ts` (`setupGameCanvas`: fixed
...
18. ✅ `render/dungeon-logic.ts` — pure rules extracted from the dungeon
...
19. ✅ `render/dungeon.ts` — the full dungeon renderer moved out of game.js
...
20. ✅ `render/town.ts` — the town renderer: backdrop, parallax ceiling,
...
- ~~`render/` — canvas setup/scaling plus the town/dungeon drawing functions~~ ✅ (items 17–20)
- ~~Input event *routing* (modal/inventory/conversation dispatch) → `core/`~~ ✅ (item 16)
- ~~Conversation system, Rokademo demo, town-transition flow~~ ✅ (items 13–15)
- ~~Game loop + boot → `main.ts`~~ ✅ (item 20)

21. ✅ `game.js` → `src/main.ts` — the monolith is gone. main.ts is now only
...
**Sub-steps** *(detailed from code survey; dungeon.c = 6.5k lines)*:
...
Detailed steps *(from code survey; the AI entry point is
...
build green; game plays identically; save/load works.
